...
const PERIODS = [
  ...
  { value: "T1", label: "T1 (Janv-Mars)" },
  { value: "T2", label: "T2 (Avril-Juin)" },
  { value: "T3", label: "T3 (Juil-Sept)" },
...
export default function AccountingTabs({
  ...
  const monthlyData = useMemo(() => {
    ...
    recipes.forEach((r) => {
      if (r.date) data[new Date(r.date).getMonth()].recipes += r.amount;
    });
    purchases.forEach((p) => {
      if (p.date) data[new Date(p.date).getMonth()].purchases += p.amount;
    });
    return data;
  }, [recipes, purchases]);
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3 text-[11px]">
              <span className="flex items-center gap-1.5">
                ...
                <span className="text-zinc-500">Encaissements</span>
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-sm bg-red-400/50" />
                <span className="text-zinc-500">Achats</span>
              </span>
            </div>
          </div>
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center justify-end gap-0.5 group relative">
                  <div className="absolute -top-2 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded px-2 py-1 text-[10px] z-10 shadow-lg">
                    ...
                    <p className="text-red-400">Achats : {formatCurrency(m.purchases)}</p>
                  </div>
          ...
          <div className="overflow-x-auto">
            {filteredPurchases.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-zinc-500 text-sm">Aucun achat sur cette periode</p>
              </div>
            ) : (
              <table className="w-full text-[13px]">
                <thead>
                ...
                <tfoot>
                  <tr className="bg-[var(--color-bg-hover)] border-t-2 border-[var(--color-border)]">
                    <td colSpan={5} className="px-5 py-3 text-sm font-semibold text-zinc-400 text-right">Total achats</td>
