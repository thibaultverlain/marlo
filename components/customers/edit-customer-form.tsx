...
export default function EditCustomerForm({ customer }: { customer: Customer }) {
  ...
  const [preferredBrands, setPreferredBrands] = useState<string[]>(customer.preferredBrands ?? []);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    Object.entries(form).forEach(([k, v]) => {
      if (typeof v === "boolean") { if (v) formData.append(k, "on"); }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-[var(--color-bg-card)] rounded-[14px] border border-[var(--color-border)] shadow-[var(--shadow-card)] p-6 space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div><label className={labelClass}>Prenom *</label><input type="text" required value={form.firstName} onChange={(e) => updateField("firstName", e.target.value)} className={inputClass} /></div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div><label className={labelClass}>Telephone</label><input type="tel" value={form.phone} onChange={(e) => updateField("phone", e.target.value)} placeholder="06 12 34 56 78" className={inputClass} /></div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div><label className={labelClass}>Adresse</label><input type="text" value={form.address} onChange={(e) => updateField("address", e.target.value)} placeholder="Adresse complete" className={inputClass} /></div>

        <div className="border-t border-[var(--color-border)] pt-5">
          <h3 className="text-sm font-semibold text-zinc-300 mb-4">Preferences</h3>
          <div className="relative">
            <label className={labelClass}>Marques favorites</label>
            <div className="flex flex-wrap gap-1.5 mb-2">
              {preferredBrands.map((b) => (
                <span key={b} className="inline-flex items-center gap-1 px-2 py-1 bg-[var(--color-bg-hover)] rounded text-xs text-zinc-300">
                  {b}
                  <button type="button" onClick={() => setPreferredBrands(preferredBrands.filter((x) => x !== b))} className="hover:text-red-400">
                    <X size={12} />
                  </button>
                </span>
              ))}
            </div>
            <input type="text" value={brandInput} onChange={(e) => { setBrandInput(e.target.value); setShowBrandDropdown(true); }}
              onFocus={() => brandInput && setShowBrandDropdown(true)}
              onBlur={() => setTimeout(() => setShowBrandDropdown(false), 200)}
              placeholder="Ajouter..." className={inputClass} />
            {showBrandDropdown && filteredBrands.length > 0 && (
              <div className="absolute z-10 w-full mt-1 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-lg shadow-2xl max-h-40 overflow-y-auto">
                {filteredBrands.map((b) => (
                  <button key={b} type="button" onMouseDown={() => { setPreferredBrands([...preferredBrands, b]); setBrandInput(""); setShowBrandDropdown(false); }}
                    className="w-full text-left px-3 py-2 text-sm text-zinc-300 hover:bg-[var(--color-bg-hover)]">
                    {b}
                  </button>
                ))}
              </div>
          ...
          <div className="mt-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.vip} onChange={(e) => updateField("vip", e.target.checked)}
                className="rounded border-[var(--color-border)] bg-[var(--color-bg)] text-rose-500 focus:ring-rose-500" />
              <span className="text-sm text-zinc-300 flex items-center gap-1">
                Client VIP{form.vip && <Star size={12} className="text-amber-400 fill-amber-400" />}
              </span>
            </label>
          </div>
        </div>
        ...
        <div className="border-t border-[var(--color-border)] pt-5">
          ...
          <textarea value={form.notes} onChange={(e) => updateField("notes", e.target.value)} rows={4}
            placeholder="Preferences, habitudes..." className={`${inputClass} resize-none`} />
        </div>
      </div>

      <div className="flex items-center justify-end gap-3 pb-8">
        <Link href={`/customers/${customer.id}`} className="px-4 py-2.5 text-sm font-medium text-zinc-500 hover:text-zinc-300 transition-colors">
          Annuler
        </Link>
