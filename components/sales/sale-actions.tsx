...
export default function SaleActions({ saleId }: { saleId: string }) {
  ...
  return (
    <div>
      ...
      <button onClick={handleGenerate} disabled={isPending} className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-rose-500 rounded-lg hover:bg-rose-400 transition-colors disabled:opacity-50"><FileText size={16} />{isPending ? "Génération..." : "Générer la facture"}</button>
    </div>
  );
}
