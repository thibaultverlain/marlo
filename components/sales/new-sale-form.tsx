    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-[var(--color-bg-card)] rounded-[14px] border border-[var(--color-border)] shadow-[var(--shadow-card)] p-6 space-y-5">
        ...
        <div className="border-t border-[var(--color-border)] pt-5">
          ...
          <div><label className={labelClass}>Prix de vente *</label><div className="relative"><input type="number" step="0.01" required value={form.salePrice} onChange={(e) => updateField("salePrice", e.target.value)} placeholder="0.00" className={`${inputClass} pr-8`} /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-zinc-500">€</span></div></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
            <div><div className="flex items-center justify-between mb-1.5"><label className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider">Frais plateforme</label><label className="flex items-center gap-1.5 cursor-pointer"><input type="checkbox" checked={form.platformFeesAuto} onChange={(e) => updateField("platformFeesAuto", e.target.checked)} className="rounded border-[var(--color-border)] bg-[var(--color-bg)] text-rose-500 focus:ring-rose-500" /><span className="text-[10px] text-zinc-500">Auto</span></label></div><div className="relative"><input type="number" step="0.01" value={form.platformFeesAuto ? autoFees.toFixed(2) : form.platformFees} onChange={(e) => updateField("platformFees", e.target.value)} disabled={form.platformFeesAuto} className={`${inputClass} pr-8 disabled:opacity-50`} /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-zinc-500">€</span></div></div>
            <div><label className={labelClass}>Frais de port</label><div className="relative"><input type="number" step="0.01" value={form.shippingCost} onChange={(e) => updateField("shippingCost", e.target.value)} placeholder="0.00" className={`${inputClass} pr-8`} /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-zinc-500">€</span></div></div>
          </div>
          <div className="mt-3"><label className={`${labelClass} mb-2`}>Port payé par</label><div className="flex gap-2">{[{v:"acheteur",l:"Acheteur"},{v:"vendeur",l:"Moi"},{v:"offert",l:"Offert"}].map((o) => <button key={o.v} type="button" onClick={() => updateField("shippingPaidBy", o.v)} className={`px-3 py-1.5 text-[13px] rounded-lg border transition-colors ${form.shippingPaidBy === o.v ? "bg-rose-500 text-white border-indigo-600" : "bg-transparent text-zinc-400 border-[var(--color-border)] hover:border-zinc-600"}`}>{o.l}</button>)}</div></div>
          {marginCalc && (
