      ...
      <div className="bg-[var(--color-bg-card)] rounded-[14px] border border-[var(--color-border)] shadow-[var(--shadow-card)] p-6 space-y-5">
        ...
        <div className="border-t border-[var(--color-border)] pt-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
            <div><div className="flex items-center justify-between mb-1.5"><label className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider">Frais plateforme</label><label className="flex items-center gap-1.5 cursor-pointer"><input type="checkbox" checked={form.platformFeesAuto} onChange={(e) => updateField("platformFeesAuto", e.target.checked)} className="rounded border-[var(--color-border)] bg-[var(--color-bg)] text-rose-500 focus:ring-rose-500" /><span className="text-[10px] text-zinc-500">Auto</span></label></div><div className="relative"><input type="number" step="0.01" value={form.platformFeesAuto ? autoFees.toFixed(2) : form.platformFees} onChange={(e) => updateField("platformFees", e.target.value)} disabled={form.platformFeesAuto} className={`${inputClass} pr-8 disabled:opacity-50`} /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-zinc-600">€</span></div></div>
