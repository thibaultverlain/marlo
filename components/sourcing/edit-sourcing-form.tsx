...
import { updateSourcingAction } from "@/lib/actions/sourcing";
...
export default function EditSourcingForm({
  ...
  const initialCommission = sourcing.commissionRate ? (Number(sourcing.commissionRate) * 100).toString() : "15";
  const [form, setForm] = useState({
    customerId: sourcing.customerId ?? "",
    description: sourcing.description ?? "",
    brand: sourcing.brand ?? "",
    model: sourcing.model ?? "",
    targetBudget: sourcing.targetBudget ?? "",
    commissionRate: initialCommission,
    deadline: sourcing.deadline ? new Date(sourcing.deadline).toISOString().split("T")[0] : "",
  ...
  const filteredBrands = useMemo(
    () => (!brandSearch ? [] : [...new Set(LUXURY_BRANDS)].filter((b) => b.toLowerCase().includes(brandSearch.toLowerCase())).slice(0, 6)),
    [brandSearch]
  );

  const commPreview = useMemo(() => {
    const b = parseFloat(form.targetBudget) || 0;
    const r = parseFloat(form.commissionRate) || 0;
    return b > 0 && r > 0 ? (b * r) / 100 : null;
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const r = await updateSourcingAction(sourcing.id, fd);
      if (r?.error) setError(r.error);
      else router.push(`/sourcing/${sourcing.id}`);
    });
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-[var(--color-bg-card)] rounded-[14px] border border-[var(--color-border)] shadow-[var(--shadow-card)] p-6 space-y-5">
        <div>
          ...
          <select required value={form.customerId} onChange={(e) => updateField("customerId", e.target.value)} className={inputClass}>
            <option value="">Pour qui ?</option>
            {customers.map((c) => (
              <option key={c.id} value={c.id}>{c.vip ? "* " : ""}{c.name}</option>
            ))}
          </select>
        </div>
        <div>
          <label className={labelClass}>Description *</label>
          <input type="text" required value={form.description} onChange={(e) => updateField("description", e.target.value)}
            placeholder="Ex: Birkin 25 Gold Togo" className={inputClass} />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="relative">
            <label className={labelClass}>Marque</label>
            <input type="text" value={brandSearch || form.brand}
              onChange={(e) => { setBrandSearch(e.target.value); updateField("brand", e.target.value); setShowDD(true); }}
              onFocus={() => brandSearch && setShowDD(true)}
              onBlur={() => setTimeout(() => setShowDD(false), 200)}
              placeholder="Hermes..." className={inputClass} />
            {showDD && filteredBrands.length > 0 && (
              <div className="absolute z-10 w-full mt-1 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-lg shadow-2xl max-h-48 overflow-y-auto">
                {filteredBrands.map((b) => (
                  <button key={b} type="button" onMouseDown={() => { updateField("brand", b); setBrandSearch(b); setShowDD(false); }}
          ...
          <div>
            ...
            <input type="text" value={form.model} onChange={(e) => updateField("model", e.target.value)} placeholder="Birkin 25..." className={inputClass} />
          </div>
        </div>
        <div className="border-t border-[var(--color-border)] pt-5">
          <h3 className="text-sm font-semibold text-zinc-300 mb-4">Budget et commission</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
          ...
          {commPreview !== null && (
            <div className="mt-3 p-3 bg-rose-500/[0.08] border border-rose-500/20 rounded-lg text-sm text-rose-300">
              Commission estimee : <span className="font-semibold">{formatCurrency(commPreview)}</span>
            </div>
          )}
        </div>
        <div>
        ...
        <div>
          ...
          <textarea value={form.notes} onChange={(e) => updateField("notes", e.target.value)} rows={3}
            placeholder="Precisions..." className={`${inputClass} resize-none`} />
        </div>
      </div>
      <div className="flex items-center justify-end gap-3 pb-8">
        <Link href={`/sourcing/${sourcing.id}`} className="px-4 py-2.5 text-sm font-medium text-zinc-500 hover:text-zinc-300 transition-colors">
          Annuler
        </Link>
