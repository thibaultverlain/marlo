  ...
  return (
    <>
      ...
      {showLink && <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={()=>setShowLink(false)}><div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl max-w-lg w-full p-6 space-y-4" onClick={(e)=>e.stopPropagation()}>
        ...
        {prevComm>0&&<div className="p-3 bg-cyan-500/[0.08] border border-indigo-500/20 rounded-lg text-sm text-cyan-300">Commission : <span className="font-semibold">{formatCurrency(prevComm)}</span></div>}
        <div className="flex justify-end gap-2 pt-2"><button onClick={()=>setShowLink(false)} className="px-4 py-2 text-sm text-zinc-500 hover:text-zinc-300">Annuler</button><button onClick={handleLink} disabled={!selProd||!pp||!sp||isPending} className="px-4 py-2 text-sm font-medium text-white bg-cyan-500 rounded-lg hover:bg-cyan-400 disabled:opacity-50">{isPending?"...":"Lier"}</button></div>
      </div></div>}
    </>
  );
}
