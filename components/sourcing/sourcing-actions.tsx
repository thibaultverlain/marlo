  ...
  return (
    <>
      {!hasFoundProduct && ["ouvert","en_recherche"].includes(status) && availableProducts.length>0 && (
        <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-5"><h3 className="text-sm font-semibold text-zinc-300 mb-2">Pièce trouvée ?</h3><p className="text-[11px] text-zinc-500 mb-4">Lie un article du stock pour calculer la commission.</p>
      ...
      {showLink && <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={()=>setShowLink(false)}><div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl max-w-lg w-full p-6 space-y-4" onClick={(e)=>e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-3"><div><label className="block text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-1.5">Prix achat</label><input type="number" step="0.01" value={pp} onChange={(e)=>setPP(e.target.value)} className={inputClass}/></div><div><label className="block text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-1.5">Prix client</label><input type="number" step="0.01" value={sp} onChange={(e)=>setSP(e.target.value)} className={inputClass}/></div></div>
        {prevComm>0&&<div className="p-3 bg-indigo-500/[0.08] border border-indigo-500/20 rounded-lg text-sm text-indigo-300">Commission : <span className="font-semibold">{formatCurrency(prevComm)}</span></div>}
        <div className="flex justify-end gap-2 pt-2"><button onClick={()=>setShowLink(false)} className="px-4 py-2 text-sm text-zinc-500 hover:text-zinc-300">Annuler</button><button onClick={handleLink} disabled={!selProd||!pp||!sp||isPending} className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-500 disabled:opacity-50">{isPending?"...":"Lier"}</button></div>
      </div></div>}
    </>
  );
}
