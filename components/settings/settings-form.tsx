  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-[var(--color-bg-card)] rounded-[14px] border border-[var(--color-border)] shadow-[var(--shadow-card)] p-6 space-y-5">
        ...
        <label className="flex items-start gap-3 cursor-pointer"><input type="checkbox" checked={form.vatSubject} onChange={(e) => updateField("vatSubject", e.target.checked)} className="mt-1 rounded border-[var(--color-border)] bg-[var(--color-bg)] text-rose-500 focus:ring-rose-500" /><div><p className="text-sm text-zinc-200">Je suis assujetti à la TVA</p><p className="text-[11px] text-zinc-500 mt-0.5">Si décoché : mention "TVA non applicable, art. 293 B du CGI" automatique sur les factures.</p></div></label>
      ...
      <div className="flex items-center justify-end gap-3 pb-8">
        <button type="submit" disabled={isPending} className="flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-rose-500 rounded-lg hover:bg-rose-400 transition-colors disabled:opacity-50"><Save size={16} />{isPending ? "..." : "Enregistrer"}</button>
      </div>
    </form>
  );
}
