...
import { updateMissionAction } from "@/lib/actions/personal-shopping";
import type { PersonalShoppingMission } from "@/lib/db/schema";
...
export default function EditMissionForm({ mission }: { mission: PersonalShoppingMission }) {
  const router = useRouter();
  const [form, setForm] = useState({
    name: mission.name,
    eventDate: mission.eventDate ? new Date(mission.eventDate).toISOString().split("T")[0] : "",
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const r = await updateMissionAction(mission.id, fd);
      if (r?.error) setError(r.error);
      else router.push(`/personal-shopping/${mission.id}`);
    });
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-[var(--color-bg-card)] rounded-[14px] border border-[var(--color-border)] shadow-[var(--shadow-card)] p-6 space-y-5">
        <div>
          ...
          <input type="text" required value={form.name} onChange={(e) => updateField("name", e.target.value)}
            placeholder="Ex: Vente privee Dior avril" className={inputClass} />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className={labelClass}>Date</label>
            <input type="date" value={form.eventDate} onChange={(e) => updateField("eventDate", e.target.value)} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Lieu</label>
            <input type="text" value={form.location} onChange={(e) => updateField("location", e.target.value)}
              placeholder="Dior Avenue Montaigne" className={inputClass} />
          </div>
        </div>
        <div>
      ...
      <div className="flex items-center justify-end gap-3 pb-8">
        <Link href={`/personal-shopping/${mission.id}`} className="px-4 py-2.5 text-sm font-medium text-zinc-500 hover:text-zinc-300 transition-colors">
          Annuler
        </Link>
