...
import { deleteMissionAction } from "@/lib/actions/personal-shopping";

export default function MissionHeaderActions({ missionId, missionName }: { missionId: string; missionName: string }) {
  ...
  return (
    <>
      <div className="flex items-center gap-1.5 flex-wrap">
        <Link
          href={`/personal-shopping/${missionId}/edit`}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setConfirming(false)}>
          <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-5 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h3 className="text-[15px] font-semibold text-white">Supprimer cette mission</h3>
                <p className="text-[12px] text-zinc-500 mt-0.5 truncate max-w-[280px]">{missionName}</p>
              </div>
            ...
            <p className="text-[13px] text-zinc-400 mb-5">
              La mission et tous les articles associes seront supprimes definitivement.
            </p>
