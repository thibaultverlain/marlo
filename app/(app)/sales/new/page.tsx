...
export default async function NewSalePage({ searchParams }: { searchParams: Promise<{ productId?: string }> }) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      {products.length === 0 ? (
        <div className="card-static p-10 text-center"><p className="text-zinc-500 mb-4 text-sm">Aucun article en stock.</p><Link href="/products/new" className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-cyan-500 rounded-lg hover:bg-cyan-400 transition-colors">Ajouter un article</Link></div>
      ) : (
