  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 lg:gap-4">
        <div className="card-static p-5"><div className="flex items-center gap-2 mb-1"><TrendingUp size={14} className="text-emerald-400" /><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider">Recettes {year}</p></div><p className="text-2xl font-semibold text-white tabular-nums">{formatCurrency(stats.revenue)}</p><p className="text-[11px] text-zinc-600 mt-0.5">{stats.salesCount} vente{stats.salesCount>1?"s":""}</p></div>
        <div className="card-static p-5"><div className="flex items-center gap-2 mb-1"><TrendingDown size={14} className="text-red-400" /><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider">Dépenses {year}</p></div><p className="text-2xl font-semibold text-white tabular-nums">{formatCurrency(stats.expenses)}</p></div>
