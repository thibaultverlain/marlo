  ...
  return (
    <div className="space-y-6 page-enter">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl lg:text-3xl font-bold text-white tracking-tight">Comptabilité</h1><p className="text-zinc-500 mt-1 text-sm">Livre de recettes et registre des achats</p></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="kpi-card p-5 flex flex-col justify-between min-h-[110px]"><div className="flex items-center gap-2 mb-1"><TrendingUp size={14} className="text-emerald-400" /><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider">Recettes {year}</p></div><p className="text-[24px] font-bold text-white tabular-nums mt-auto">{formatCurrency(stats.revenue)}</p><p className="text-[11px] text-zinc-600 mt-0.5">{stats.salesCount} vente{stats.salesCount>1?"s":""}</p></div>
        <div className="kpi-card p-5 flex flex-col justify-between min-h-[110px]"><div className="flex items-center gap-2 mb-1"><TrendingDown size={14} className="text-red-400" /><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider">Depenses {year}</p></div><p className="text-[24px] font-bold text-white tabular-nums mt-auto">{formatCurrency(stats.expenses)}</p></div>
        <div className="kpi-card p-5 flex flex-col justify-between min-h-[110px]"><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-1">Resultat brut</p><p className={`text-[24px] font-bold tabular-nums mt-auto ${benefit>=0?"text-emerald-400":"text-red-400"}`}>{formatCurrency(benefit)}</p></div>
      </div>
