...
export default async function NewSourcingPage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      {customers.length === 0 ? <div className="card-static p-10 text-center"><p className="text-zinc-500 mb-4 text-sm">Aucun client.</p><Link href="/customers/new" className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-cyan-500 rounded-lg hover:bg-cyan-400 transition-colors">Créer un client</Link></div> : <NewSourcingForm customers={customers.map((c) => ({ id: c.id, name: `${c.firstName} ${c.lastName}`, vip: c.vip ?? false }))} />}
    </div>
  );
}
