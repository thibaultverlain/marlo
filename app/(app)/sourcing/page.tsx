  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl text-white">Sourcing</h1><p className="text-zinc-500 mt-1 text-sm">{stats.active} en cours{stats.totalCommissions > 0 && ` · ${formatCurrency(stats.totalCommissions)} commissions`}</p></div>
      ...
      {requests.length === 0 ? <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-12 text-center"><Search size={40} className="mx-auto text-zinc-700 mb-3" /><p className="text-zinc-500 mb-4 text-sm">Aucune demande</p><Link href="/sourcing/new" className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-500 transition-colors"><Plus size={14} />Créer</Link></div> : (
        <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] overflow-hidden"><div className="divide-y divide-[var(--color-border)]">{requests.map((req) => {
          const st = SM[req.status ?? "ouvert"] || SM.ouvert; const Icon = st.icon;
          return <Link key={req.id} href={`/sourcing/${req.id}`} className="flex items-center justify-between px-5 py-3.5 hover:bg-[var(--color-bg-hover)] transition-colors">
