  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 lg:gap-4">
        ...
        <div className="card-static p-5"><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-1">Commission</p><p className="text-2xl font-semibold text-cyan-400">{commissionPct > 0 ? `${commissionPct.toFixed(0)}%` : "—"}</p>{req.commissionAmount && <p className="text-[11px] text-zinc-500 mt-0.5">{formatCurrency(req.commissionAmount)}</p>}</div>
      ...
      <div className="card-static p-6">
        ...
        {req.customerId && req.customerName ? <Link href={`/customers/${req.customerId}`} className="flex items-center justify-between p-3 -m-3 rounded-lg row-hover transition-colors"><p className="text-sm text-zinc-200">{req.customerName}</p><span className="text-xs text-cyan-400">Voir →</span></Link> : <p className="text-sm text-zinc-500">Inconnu</p>}
      </div>
