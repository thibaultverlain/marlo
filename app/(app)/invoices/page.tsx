...
export default async function InvoicesPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl lg:text-3xl text-white">Factures</h1><p className="text-zinc-500 mt-1 text-sm">{stats.total} facture{stats.total > 1 ? "s" : ""} · {formatCurrency(stats.totalAmount)} facturés</p></div>
      {stats.total > 0 && <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">{[{l:"Total",v:stats.total},{l:"Brouillons",v:stats.draft},{l:"Envoyées",v:stats.sent,c:"text-blue-400"},{l:"Payées",v:stats.paid,c:"text-emerald-400"}].map((s) => <div key={s.l} className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-4"><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider">{s.l}</p><p className={`text-2xl font-semibold mt-1 ${s.c ?? "text-white"}`}>{s.v}</p></div>)}</div>}
