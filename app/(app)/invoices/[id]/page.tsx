  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 lg:gap-4">
        <div className="card-static p-5"><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-1">Client</p><p className="text-lg font-semibold text-white">{customer ? `${customer.firstName} ${customer.lastName}` : "Sans client"}</p></div>
        <div className="card-static p-5"><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-1">Montant TTC</p><p className="text-2xl font-semibold text-white tabular-nums">{formatCurrency(invoice.amountTtc)}</p>{invoice.vatMention && <p className="text-[10px] text-zinc-600 mt-1">{invoice.vatMention}</p>}</div>
