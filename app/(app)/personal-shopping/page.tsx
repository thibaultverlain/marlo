  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start sm:items-center justify-between gap-3">
        ...
        <Link href="/personal-shopping/new" className="flex items-center gap-2 px-4 py-2 text-[13px] font-medium text-white bg-cyan-500 rounded-lg hover:bg-cyan-400 transition-colors"><Plus size={14} />Nouvelle mission</Link>
      </div>
      {missions.length === 0 ? <div className="card-static p-12 text-center"><ShoppingBag size={40} className="mx-auto text-zinc-700 mb-3" /><p className="text-zinc-500 mb-4 text-sm">Aucune mission</p><Link href="/personal-shopping/new" className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-cyan-500 rounded-lg hover:bg-cyan-400 transition-colors"><Plus size={14} />Créer</Link></div> : (
        <div className="card-static overflow-hidden"><div className="divide-y divide-[var(--color-border)]">{missions.map((m) => { const st = SM[m.status??"planifie"]||SM.planifie; return (
          <Link key={m.id} href={`/personal-shopping/${m.id}`} className="flex items-start sm:items-center justify-between gap-3 px-5 py-3.5 row-hover transition-colors">
            ...
            <div className="text-right flex-shrink-0 ml-4"><p className="text-[13px] font-medium text-zinc-200 tabular-nums">{formatCurrency(m.totalPurchased)}</p>{Number(m.totalCommission??0)>0&&<p className="text-[11px] text-cyan-400">+{formatCurrency(m.totalCommission)}</p>}</div>
          </Link>); })}</div></div>
      )}
    </div>
  );
