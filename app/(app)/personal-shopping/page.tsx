    ...
    <div className="space-y-6">
      ...
      {missions.length === 0 ? <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-12 text-center"><ShoppingBag size={40} className="mx-auto text-zinc-700 mb-3" /><p className="text-zinc-500 mb-4 text-sm">Aucune mission</p><Link href="/personal-shopping/new" className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-500 transition-colors"><Plus size={14} />Créer</Link></div> : (
        <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] overflow-hidden"><div className="divide-y divide-[var(--color-border)]">{missions.map((m) => { const st = SM[m.status??"planifie"]||SM.planifie; return (
          <Link key={m.id} href={`/personal-shopping/${m.id}`} className="flex items-start sm:items-center justify-between gap-3 px-5 py-3.5 hover:bg-[var(--color-bg-hover)] transition-colors">
