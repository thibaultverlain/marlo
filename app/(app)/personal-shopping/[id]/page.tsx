  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-5"><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-1">Articles</p><p className="text-2xl font-semibold text-white">{items.length}</p></div>
        <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-5"><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-1">Total</p><p className="text-2xl font-semibold text-white tabular-nums">{formatCurrency(mission.totalPurchased)}</p></div>
        <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-5"><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-1">Commissions</p><p className="text-2xl font-semibold text-indigo-400 tabular-nums">{formatCurrency(mission.totalCommission)}</p></div>
      </div>
      ...
      {mission.notes&&<div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-6"><h2 className="text-[15px] font-semibold text-white mb-3">Notes</h2><p className="text-sm text-zinc-400 whitespace-pre-line">{mission.notes}</p></div>}
      <div className="pb-8" />
    </div>
  );
}
