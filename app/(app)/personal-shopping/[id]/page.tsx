...
export default async function MissionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [mission, items, customers] = await Promise.all([getMissionById(id), getMissionItems(id), getAllCustomers(shopId)]);
  if (!mission) notFound();
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 page-enter">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="kpi-card p-5 flex flex-col justify-between min-h-[110px]"><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider">Articles</p><p className="text-[24px] font-bold text-white mt-auto">{items.length}</p></div>
        <div className="kpi-card p-5 flex flex-col justify-between min-h-[110px]"><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider">Total</p><p className="text-[24px] font-bold text-white tabular-nums mt-auto">{formatCurrency(mission.totalPurchased)}</p></div>
        <div className="kpi-card p-5 flex flex-col justify-between min-h-[110px]"><p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider">Commissions</p><p className="text-[24px] font-bold text-rose-400 tabular-nums mt-auto">{formatCurrency(mission.totalCommission)}</p></div>
      </div>
