      ...
      {rows.length > 0 && !success && (
        <div className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-6">
          ...
          <div className="flex items-center justify-end mt-6 pt-4 border-t border-[var(--color-border)]">
            <button onClick={handleImport} disabled={isPending||validCount===0} className="flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-500 transition-colors disabled:opacity-50"><FileText size={16}/>{isPending?"Import...":` Importer ${validCount} article${validCount>1?"s":""}`}</button>
          </div>
        </div>
      )}
      <div className="pb-8" />
