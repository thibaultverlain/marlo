...
import { sales, products, customers, invoices, purchases } from "../schema";
...
export async function getRecipeBook(year?: number): Promise<RecipeEntry[]> {
  ...
  const rows = await db
    ...
    .where(and(gte(sales.soldAt, startOfYear), lte(sales.soldAt, endOfYear)))
...
export async function getPurchasesRegister(year?: number): Promise<PurchaseEntry[]> {
  const y = year ?? new Date().getFullYear();
  const startOfYear = new Date(y, 0, 1);
  const endOfYear = new Date(y, 11, 31, 23, 59, 59);

  // Get explicit purchases (table purchases)
  ...
  // Also include implicit purchases from products themselves (purchase_date field)
  const productPurchases = await db
    .select({
      id: products.id,
      purchasedAt: products.purchaseDate,
      description: products.title,
      supplier: products.purchaseSource,
      amount: products.purchasePrice,
      paymentMethod: sql<string | null>`null`,
      category: sql<string>`'stock'`,
      productSku: products.sku,
      productTitle: products.title,
    })
    .from(products)
...
export async function getAccountingStats(year?: number) {
  const y = year ?? new Date().getFullYear();
  const startOfYear = new Date(y, 0, 1);
  const endOfYear = new Date(y, 11, 31, 23, 59, 59);
  ...
  const [recipesStats, purchasesStats, productsStats] = await Promise.all([
    ...
    db
      .select({
        totalPurchases: sql<number>`coalesce(sum(purchase_price), 0)::numeric`,
        count: sql<number>`count(*)::int`,
      })
      .from(products)
      .where(
  ...
  return {
    revenue: Number(recipesStats[0]?.totalRevenue ?? 0),
    salesCount: recipesStats[0]?.count ?? 0,
